...
public class Drive extends Command {
    ...
    public void execute() {
        ...
        double leftJoyY = MathUtil.applyDeadband(Math.signum(xbox.getLeftY()) * Math.pow(xbox.getLeftY(), 2), 0.05);
        double leftJoyX = MathUtil.applyDeadband(Math.signum(xbox.getLeftX()) * Math.pow(xbox.getLeftX(), 2), 0.05);
        // double rBoxY = Math.signum(xbox.getRightY()) * Math.pow(xbox.getRightY(), 2)
        // * -1;
        // double rBoxX = Math.signum(xbox.getRightX()) * Math.pow(xbox.getRightX(), 2)
        // * -1;
